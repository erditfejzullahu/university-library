...
const BorrowedBooksList = ({bookItem}: {bookItem: BorrowedBook}) => {
  ...
  return (
    <article onClick={() => router.push(`/books/${bookItem.book.id}`)} className="bg-dark-100 cursor-pointer self-start items-center h-full justify-center rounded-xl gap-4 relative">
        ...
        <div className="w-full flex-1 flex flex-col gap-3 p-4 rounded-lg !overflow-hidden shadow-[0_0_25px_3px_rgba(0,0,0,0.65)] h-full">
            ...
            <div className="mt-2">
                <h3 className="font-ibm-plex-sans font-medium text-white text-xl line-clamp-1">{bookItem.book.title}</h3>
                <p className="text-white italic font-ibm-plex-sans text-xs line-clamp-1">{bookItem.book.genre}</p>
            </div>
